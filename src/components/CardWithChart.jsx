...
import { FaUserGroup, FaArrowDown, FaArrowUp } from "react-icons/fa";
  ...
  return (
    <Card className="dashboard_cards" style={{ border: "none" }}>
      <Card.Body>
        ...
        <h6 className={DashboardListingData?.data?.userPercentCount === 0 ? "text-muted" : (DashboardListingData?.data?.userPercentCount > 0 ? "text-success" : "text-danger")}>
  <span>
    {DashboardListingData?.data?.userPercentCount.toFixed()}
  </span>
  {DashboardListingData?.data?.userPercentCount !== 0 && (
    <span>
      {DashboardListingData?.data?.userPercentCount > 0 ? (
        <FaArrowUp />
      ) : (
