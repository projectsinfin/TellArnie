...
const export_registered_kits = async (req, res) => {
  try {
    ...
    if (is_all) {
      // Fetch all products
      products = await RegisteredKit.find({
        is_dump:false
      }).lean();
    ...
    res.setHeader('Content-disposition', 'attachment; filename=registeredKits.csv');
...
module.exports = export_registered_kits;
